...
public class Main {
    public static void main(String[] args) {
        ...
        System.out.print("\nARRAYS EXAMPLE 3:   ");
        int[] myListe = new int[20];
        int total = 0, count = 0;
        for (int i = 0; i < 20; i++){
            //add code here
            myListe[i] = rand.nextInt(11) + 10;
            if (myListe[i] > 15 && myListe[i] %2 == 0){
                total += myListe[i];
                count ++;
            }
        }
        System.out.println(Arrays.toString(myListe) + " " + total + " " + count);
        ...
        //SHUFFLE
        for (int i = 0; i < 52; i++){
            //get a random position & swap with current
            //add code here
            int pos = rand.nextInt(52);
            int holdValue = deck[pos];
            deck[pos] = deck [i];
            deck[i] = holdValue;
        }
        ...
        System.out.println(card2 + " " + cards[card2%13] + " of " + suit[card2%4]);
        for (int i = 0; i < 52; i++)
          System.out.println(deck[i] + " " + cards[deck[i]%13] + " of " + suit[deck[i]%4]);
